...
<br>
<?php
    $ss = mysql_fetch_array(mysql_query("SELECT * FROM tbl_kelas where kode_kelas='$_GET[id]'"));

    $g = mysql_fetch_array(mysql_query("SELECT * FROM tbl_siswa a JOIN tbl_kelas b ON a.kode_kelas=b.kode_kelas where a.nipd='$_GET[nipd]'"));
      if ($_GET[semester]=='1'){
        $ket = 'GANJIL';
      }else{
        $ket = 'GENAP';
    ...
    echo "<table>
        ...
        <tr><td style='background:#f4f4f4; border-bottom:1px solid #8a8a8a'><b>Kelas</b></td>    <td style='border-bottom:1px solid #8a8a8a'> : $g[nama_kelas]</td></tr>
    ...
    $tampil = mysql_query("SELECT * FROM tbl_nilai a JOIN tbl_pelajaran b ON a.kode_pelajaran=b.kode_pelajaran
                           where a.nipd='$_GET[nipd]' ORDER BY b.semester");
    $no = 1;
